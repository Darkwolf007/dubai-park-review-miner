...
`lands_design_handoff.json` prepares the later Lands Design for Rhino conversion without creating proprietary objects prematurely. It maps runtime terrain, path curves, planting zones/points, landscape zones, and point assets to intended Lands Design classes; defines stable Rhino parameter names; and records every unresolved detail-design decision.
...
The walking network is a deterministic park-path hierarchy rather than an arbitrary web of program-to-program chords. A gently curved primary accessible, shade-priority spine connects the main and secondary public entrances (or the deterministic farthest anchor pair when entrance data is unavailable) through a central social destination. Each remaining program receives one OD-prioritized, corner-smoothed secondary branch from its proxy edge to the closest point on the growing network, allowing later branches to merge into earlier ones. Jogging and cycling remain separate rounded perimeter loops, while service access remains a dedicated operations spur. Public paths add boundary-safe detour vertices around unrelated exclusive area bubbles where feasible. Accessibility and shade are stored as attributes on walking segments rather than duplicated centerlines. `Route Corridor Edges` offsets physical centerlines by half the exported width; `Route Corridor Data` reports the associated footprint, source length, layer, and stable program ID. These are width-aware schematic path geometries; terrain projection, slope checks, drainage coordination, and detailed pavement construction remain downstream design steps.

For circulation review, preview `Public Walking Paths` rather than the combined legacy `Routes` output. Walking branches now attach to the closest existing spine or previously generated branch, so the public system grows as one merging tree. `Service Paths`, perimeter `Loop Routes`, and `Other Route Systems` are exposed separately to prevent lighting, irrigation, bioswale, jogging, and cycling geometry from reading as one chaotic path network.
...
`TreemapPaths` in `Park Design > Planning` converts a touching rectangular-cartogram/treemap layout into a connected path hierarchy. Connect the packed `Cells`, `ParkBubbles.Area Program IDs`, and `ParkBubbles.Names` in matching order; optionally provide exact main, secondary, and service entrance points. The component detects shared cell boundaries as controlled program portals, locks a main entrance-to-community-hub-to-secondary-entrance spine, connects the remaining public cells with a shortest-path tree, and keeps Operations/Maintenance on an independent service route.
...
For centre-generated Voronoi layouts, connect ParkBubbles Centers to ParkCirculation Program Centers and leave Polygon Mode at AUTO. When the supplied cells cover most of the site, AUTO treats them as influence territories rather than constructed footprints. Each cell is matched back to the centre that generated it, clipped to the site, and scaled about that centre using the program target area subject to Maximum Territory Fill. The Routed Program Footprints output shows the exact smaller polygons used by routing; the space between them remains available as the park's circulation and landscape matrix. Use FOOTPRINTS only when the supplied polygons already represent their final occupied extents.

ParkBubbles exposes Node Program IDs matching Nodes and Node Names; connect these directly to ParkCirculation Point Program IDs.

`TreeEdgePaths` is a separate component in Park Design > Planning, parallel to `TreemapPaths` and `ParkCirculation`. Connect the site Boundary, original Voronoi Cells, optional matching Program IDs/Names, Main Entrance, optional Secondary Entrances, and Run. It finds shared-cell-edge portals and returns a rooted, loop-free `Tree Edge Paths` network that stays on cell boundaries rather than connecting centroids. Its portal/path metadata, snapped root, warnings, and clipped cells are exposed separately. Any off-edge entrance approach or connection from a cell edge to an inset program footprint remains a downstream detail; smoothing must also be checked against the exact cell-edge geometry.
...
The Smoothness input controls obstacle-safe geometric refinement. Zero returns the validated merged polylines. Higher values remove grid stair-stepping, apply up to three bounded corner-cutting passes, and create a chord-parameterized interpolated curve while keeping chain endpoints and graph junctions fixed. Every candidate spline is resampled against the movement grid; if it crosses a blocked region or leaves the site, the component returns the valid refined polyline instead. Raw Grid Paths and Refinement Data expose the before/after geometry and the accepted or fallback status for every path.
